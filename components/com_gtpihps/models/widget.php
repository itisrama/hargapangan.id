...
class GTPIHPSModelWidget extends GTModelList {
	...
	public function getData($type = 'commodity') {
		...
		// Join Markets
		$query->join('INNER', $db->quoteName('#__gtpihps_markets', 'd') . ' ON ' . $db->quoteName('b.market_id') . ' = ' . $db->quoteName('d.id'));
		...
		switch($type) {
			...
			case 'category':
				...
				$query->join('INNER', $db->quoteName('#__gtpihps_commodities', 'c') . ' ON ' . $db->quoteName('a.commodity_id') . ' = ' . $db->quoteName('c.id'));
	...
	public function getLatestDates() {
		...
		// Join Markets
		$query->join('INNER', $db->quoteName('#__gtpihps_markets', 'b') . ' ON ' . $db->quoteName('a.market_id') . ' = ' . $db->quoteName('b.id'));
	...
	public function getLocation($all = false, $prepare = false) {
		...
		$location		= $this->getState('filter.location');
		$price_type_id	= $this->getState('filter.price_type_id');
		$table			= '#__gtpihps_'.GTHelper::pluralize($location);
