...
class GTPIHPSModelService extends GTModelAdmin{
	...
	public function getIntegrationPrices() {
		...
		$query->join('INNER', $db->quoteName('#__gtpihps_markets_copy', 'c').' ON '.
		...
		$query->join('INNER', $db->quoteName('#__gtpihps_commodities', 'd').' ON '.
			$db->quoteName('b.commodity_id').' = '.$db->quoteName('d.id')
		);

		$query->join('INNER', $db->quoteName('#__gtpihps_provinces', 'e').' ON '.
