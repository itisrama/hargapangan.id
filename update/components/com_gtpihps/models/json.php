...
class GTPIHPSModelJSON extends GTModelList{
	...
	public function getReferences() {
		...
		$query->join('INNER', $db->quoteName('#__gtpihps_regencies', 'b').' ON '.$db->quoteName('a.regency_id').' = '.$db->quoteName('b.id'));

		$query->select($db->quoteName('c.name', 'province'));
		$query->join('INNER', $db->quoteName('#__gtpihps_provinces', 'c').' ON '.$db->quoteName('a.province_id').' = '.$db->quoteName('c.id'));
