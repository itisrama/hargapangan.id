...
class GTPIHPSControllerJson extends GTControllerForm {
	...
	public function statFluctuationByProvince() {
		...
		$model			= $this->getModel();
		$flucs			= $this->getFluctuationByProvince();
		$date			= $flucs->date;
		$periods		= $flucs->periods;
		$startDate		= reset($periods);
		$endDate		= end($periods);
	...
	public function getProvinceDetail() {
		...
		$model				= $this->getModel('Stats_Commodity');
		$all_prices			= $model->getItemsAll();
		$province_prices	= $model->getItemsProv();
		$regency_prices		= $model->getItemsReg();
		$market_prices		= $province_id > 0 ? $model->getItemsMar() : array();

		$provinces 			= $model->getProvinces();
		$regencies 			= $model->getRegencyList();

		//echo "<pre>"; print_r($regencies); echo "</pre>"; die;
		$markets 			= $province_id > 0 ? $model->getMarketList() : array();

		$downUrl	= 'index.php?option=com_gtpihps&view=stats_commodity';
		$Itemid		= GTHelper::getMenuId($downUrl);
		$downUrl	= sprintf($downUrl.'&province_id=%s&commodity_id=%s&date=%s&layout=%s&format=xls&is_external=1&Itemid=%s', $province_id, $commodity_id, $date, $layout, $Itemid);
		...
		if($province_id > 0) {
			...
			foreach ($provinces as $province_id => $province) {
				$provRegencies	= $regencies[$province_id];
				$provPrices	= $province_prices[$province_id];
				if(!count($provRegencies) > 0) continue;

				$item = array('<strong>'.$province.'</strong>');
				foreach ($dates as $date) {
					$price	= @$provPrices[$date->unix];
					$price	= $price ? '<strong>'.$price.'</strong>' : '<div class="text-center"><strong>-</strong></div>';
					$item[]	= $price;
				...
				foreach ($provRegencies as $regency_id => $regency) {
					$regPrices = $regency_prices[$regency_id];
					$item = array('<div style="padding-left:1.4em">'.$regency.'</div>');
					foreach ($dates as $date) {
						$price = @$regPrices[$date->unix];
						$price = $price ? $price : '<div class="text-center">-</div>';
						$item[] = $price;
					}
					$item[] = 'market market-'.$province_id;
					$item[] = $regency_id;
					$data[] = $item;
				}
			}
	...
	public function getData() {
		...
		foreach ($provinces as $prov) {
			...
			$area->tooltip->content	= '<strong>'.$prov->name.'</strong><br/>';
			$area->tooltip->content	.= $price ? $priceDisplay.'<small>'.$price->displaylong.'</small>' : JText::_('COM_GTPIHPS_NO_DATA');
			$area->tooltip->content	.= $rank > 0 && $rank < 6 ? '<br/><small>'.$price->dateinfo.'</small>' : null;
			
			$isPriceNum			= is_numeric($price->value);
			$province			= new stdClass();
			$province->value	= $isPriceNum ? $price->value : null;
			$province->name		= $prov->name;
			$province->display	= $price->display ? $price->display : '-';
			$province->id		= $id;
			...
			$provRefs[]						= array($id, $prov->short_name, $prov->name, $price->display);
			$areas[trim($prov->iso_code)]	= $area;
			$provData[$area->value][]		= array($id, $province->value);
		}

		sort($provList);
		sort($provEmpty);
		$tableData = array_merge($provList, $provEmpty);
		...
		$result->provinces		= $provRefs;
		$result->provData		= $provData;
	...
	protected function getFluctuationByProvince() {	
		$data_type	= $this->input->get('data_type', 'fluctuation');
